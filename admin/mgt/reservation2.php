...
if(isset($_POST['Submit']))
 ...
 $r_count = mysqli_num_rows($query1);
 $sql3 = "SELECT * FROM tblroom where RoomType = '$r_type'";
 $query = mysqli_query($db, $sql3);
 $r_count2 = mysqli_num_rows($query);
if ($r_count2 == $r_count){
$msg = "Sorry, you cannot add rooms in the selected room category.";	
} else {
if(isset($_FILES['files'])){
	...
	foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
        ...
        $query="INSERT INTO tblroom (ID, RoomType, RoomNumber, MaxAdult, MaxChild, NoofBed, Image, file_name, file_size, file_type, CreationDate) VALUES ('', '$r_type', '$r_no', '$m_adult', '$m_child', '$n_bed', '', '$file_name', '$file_size', '$file_type', CURRENT_TIMESTAMP)";
        $desired_dir="Rooms_image";
...
<option value="">Choose Room Number</option>
                                                ...
                                                <input class="form-control" id="cdate" name="cdate" type="datetime-local" required />
                                                <label for="cdate">Check in date</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="cout" name="cout" type="datetime-local" required />
                                                <label for="m_child">Check out date</label>
                                            </div>  
                                            <div class="form-floating mb-3">
                                                <h4 style="font-weight: bold">Total Days : <span id="staying_day">0</span> Days</h4>
                                    <h4 style="font-weight: bold">Price: <span id="price"><?php echo $myprice; ?></span> /-</h4>
