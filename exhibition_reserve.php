...
$sql="insert into exhibition(space,price,cat,name,email,phone,address)";
$query=$dbh->prepare($sql);
$query->bindParam(':space',$space,PDO::PARAM_STR);
...
$query->execute();
   ...
   if ($LastInsertId>0) {
    echo '<script>alert("Space Reserved Successfully.")</script>';
...
<div class="women_main">
	...
	<form action="exhibition_reserve.php" method="post" enctype="multipart/form-data">
...
{          
    ?>  
<option value="<?php echo htmlentities($row->id);?>"><?php echo htmlentities($row->myspace);?> @ <?php echo htmlentities($row->price);?></option>
 <?php } ?> 
              </select> </div>
