...
session_start();
 ...
 $address=$_POST['address'];
   $e_id = mt_rand(1000000, 9999999);
 $sql1 = "SELECT * FROM space where myspace = '$space'";
$query1 = mysqli_query($db,$sql1);
while($rows = mysqli_fetch_assoc($query1)){
$price = $rows['price'];
}
 $sql3 = "INSERT INTO exhibition (id, e_id, space, price, cat, name, email, phone, address, date, status) VALUES ('', '$e_id', '$space', '$price', '$cat', '$name', '$email', '$phone', '$address', CURRENT_TIMESTAMP, 'RESERVED')";
$result = mysqli_query($db, $sql3);

if($result){
	$_SESSION['e_id'] = $e_id;
	header('location:r_invoice.php');
